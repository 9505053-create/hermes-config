...
2. **Google News 精準查詢只作輔助**，分成 FCC / CE-OJ-RED / NCC 三條，不用單純 `WiFi OR Bluetooth`。優先用 `site:fcc.gov OR site:docs.fcc.gov`、`site:eur-lex.europa.eu OR site:ec.europa.eu OR site:op.europa.eu OR site:etsi.org`、`site:ncc.gov.tw OR site:ncclaw.ncc.gov.tw` 限縮到官方/法規來源；避免 Yahoo/產品媒體用同樣關鍵字混進來。
...
const productTerms = /(WLAN|Wi-?Fi|802\.11|Bluetooth|\bBT\b|BLE|WWAN|LTE|5G|\bNR\b|cellular|U-NII|6\s?GHz|mobile broadband|藍牙|行動寬頻|低功率射頻器材|電信管制射頻器材)/i;
const regBlock = /(NRA|rifle|firearm|gun|amateur radio|broadcast|radio station|FM transmitter|AM transmitter|licensee|permit|tower|false alerts?|hijack|transmitter move|wireless device.*FCC|FCC.*wireless device|mystery|slides through FCC|ear buds?|耳機推薦|Submarine Cable|Cable Landing|\bSCL\b|新品|發表|上市|開箱|評測|實測|傳聞|爆料|售價|價格|規格|促銷|優惠|食品安全|教育.*認證|ISO.*認證)/i;
// 必須 hasReg && hasProduct；FCC 官方也不可只因來源含 FCC 就通過。
// 2026-05-14 Scott 校正：WLAN/WWAN/BT 是限縮條件，不是 RF 欄主題；也不要把 generic transmitter/radio station/FCC listing 當合格訊號。寧可 RF=0，也不要用一般產品新聞補位。
```
...
27. **Postgres 空庫/DB reset 復原** — 若 `workflow_entity`/`credentials_entity` 變成 0 筆、API 回 401、log 顯示 `Processed 0 draft workflows, 0 published workflows`，代表 n8n 可能啟動到新空 Postgres。先 `pg_dump` 做回滾備份，再從 `_3AI_WorkSpace/backups/` 匯入 workflow/credential。**不要信任備份檔名；先檢查備份內容是否真的含資料**：SQL dump 需確認 `COPY public.workflow_entity`、`COPY public.credentials_entity` 下面有資料列；若 workflow/credential 都是 0 筆，該 dump 不是可用完整備份。單一 workflow JSON 只能恢復流程結構，不能恢復 SMTP/API credential secrets。`import:credentials` 的 JSON 必須是**陣列**；`import:workflow` 若遇到 `activeVersionId` 外鍵錯誤，改用精簡 workflow JSON，只保留 `id/name/nodes/connections/settings`。匯入後用 `n8n update:workflow --id=... --active=true`，再 `docker compose restart n8n`，因 CLI 會提示 running server 下變更需重啟才生效。

28. **n8n 2.x CLI 手動執行限制** — `docker exec n8n n8n execute --id=...` 會和正在跑的 server 的 Task Broker port 5679 衝突；改用 `docker stop n8n` → `cd /mnt/c/docker/n8n && docker compose run --rm --no-deps n8n execute --id=WORKFLOW_ID` → `docker start n8n`。另外 n8n 2.x CLI execute 若 workflow 只有 Schedule Trigger，可能報 `Missing node to start execution`；可在 workflow 內保留一個 `n8n-nodes-base.executeWorkflowTrigger` 節點連到主流程，作為 CLI/救援手動執行入口，不影響每日排程。
29. **API key 401 after empty DB** — 若 DB reset 後 API key 失效，先用 UI 完成 owner setup（`userManagement.isInstanceOwnerSetUp=true`），在 Settings → n8n API 建立新 key，再更新 `~/.hermes/.env` 的 `N8N_API_KEY`。建立後必須用 `GET /api/v1/workflows?limit=5` 做 smoke test。注意 cron/script 讀 `.env` 時不要用 `os.environ.setdefault`，否則會保留舊環境中的失效 key。
